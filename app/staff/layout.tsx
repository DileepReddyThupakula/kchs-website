...
export default async function StaffLayout({ children }: { children: ReactNode }) {
  ...
  return <main className="staff-shell"><aside className="staff-sidebar"><Link href="/staff" className="staff-brand" aria-label="Krishna Chaitanya High School staff portal"><Image src="/images/krishna-chaitanya-logo.png" alt="Krishna Chaitanya High School" width={64} height={64} priority /><span><strong>Krishna Chaitanya</strong><small>HIGH SCHOOL</small><em>Staff Portal</em></span></Link><StaffNavigation /><div className="staff-sidebar-footer"><div className="staff-account"><span aria-hidden="true">{staff.role === "admin" ? "AD" : "ST"}</span><div><strong>{staff.email ?? "Authorised staff"}</strong><small>{staff.role === "admin" ? "Administrator" : "Staff member"}</small></div></div><form action={signOutStaff}><button type="submit" aria-label="Sign out">Sign out <span aria-hidden="true">→</span></button></form></div></aside><section className="staff-main">{children}</section></main>;
}
