...
export function AcademicAssignmentForm({ years, classes, sections, subjects, teachers, action }: { years: AcademicYear[]; classes: SchoolClass[]; sections: AcademicSection[]; subjects: Subject[]; teachers: StaffMember[]; action: (formData: FormData) => void | Promise<void> }) {
  const currentYear = years.find((year) => year.status === "current")?.id ?? years[0]?.id ?? "";
  ...
  const [classId, setClassId] = useState(availableSections[0]?.class_id ?? "");
  const sectionsForClass = useMemo(() => availableSections.filter((section) => section.class_id === classId), [availableSections, classId]);

  return <form className="academic-form academic-form-assignment" action={action}>
    ...
    <label>Section<select name="academic_section_id" disabled={!sectionsForClass.length} required defaultValue=""><option value="" disabled>Select section</option>{sectionsForClass.map((section) => <option key={section.id} value={section.id}>{section.name}</option>)}</select></label>
    <label>Subject<select name="subject_id" disabled={!subjects.filter((subject) => subject.active).length} required defaultValue=""><option value="" disabled>Select subject</option>{subjects.filter((subject) => subject.active).map((subject) => <option key={subject.id} value={subject.id}>{subject.name}{subject.code ? ` · ${subject.code}` : ""}</option>)}</select></label>
    <label>Teacher <span>Eligible active teaching staff only</span><select name="teacher_id"><option value="">Unassigned</option>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.full_name}{teacher.designation ? ` · ${teacher.designation}` : ""}</option>)}</select></label>
    <div className="academic-form-actions"><AcademicSubmitButton className="staff-action-submit">Save assignment</AcademicSubmitButton></div>
  </form>;
}
