...
export function StaffLoginForm() {
  ...
  return <form action={formAction} className={styles.form} aria-busy={pending}>
    ...
    <button className={styles.submit} aria-disabled={pending} disabled={pending} type="submit">{pending ? <span className={styles.spinner} aria-hidden="true" /> : <svg aria-hidden="true" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="10" width="14" height="10" rx="2" /><path d="M8 10V7a4 4 0 0 1 8 0v3" /></svg>}{pending ? "Signing in…" : "Sign in securely"}</button>
    <p className={styles.srOnly} aria-live="polite">{pending ? "Signing in, please wait." : ""}</p>
