...
type Props = StudentEnrollmentData & { initial?: Initial; submitLabel?: string; cancelHref?: string; showActions?: boolean; errors?: StudentFieldErrors; onFieldChange?: (field: string) => void };
const errorId = (field: string) => `student-${field.replaceAll("_", "-")}-error`;

export function StudentEnrollmentFields({ years, classes, sections, initial, submitLabel = "Save enrollment", cancelHref, showActions = true, errors = {}, onFieldChange }: Props) {
  ...
  return <section className="student-form-section"><header><p className="academic-kicker">Academic enrollment</p><h2>Enrollment</h2><p>Academic placement is stored separately from the student profile.</p></header><div className="student-form-grid">
    ...
    {initial ? field("enrollment_status", "Enrollment status", <select id="student-enrollment-status" name="enrollment_status" defaultValue={initial.status}><option value="active">Active</option><option value="inactive">Inactive</option><option value="completed">Completed</option><option value="transferred">Transferred</option></select>) : <input type="hidden" name="enrollment_status" value="active" />}
