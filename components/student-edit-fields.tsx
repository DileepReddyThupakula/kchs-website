...
function PersonalInput({ student, name, label, type = "text", required, inputMode, maxLength, submittedValues, fieldErrors }: PersonalFieldProps) {
  ...
  return <label className={error ? "student-field-error" : undefined} htmlFor={id}>{label}{required ? " *" : ""}<input aria-describedby={error ? `${id}-error` : undefined} aria-invalid={Boolean(error)} defaultValue={submittedValue(student, name, submittedValues)} id={id} inputMode={inputMode} maxLength={maxLength} name={name} required={required} type={type}/>{error ? <span className="student-field-message" id={`${id}-error`}>{error}</span> : null}</label>;
}

function PersonalSelect({ student, name, label, children, required, submittedValues, fieldErrors }: Omit<PersonalFieldProps, "type" | "inputMode" | "maxLength"> & { children: ReactNode }) {
  const error = fieldErrors?.[name]?.[0];
  const id = `student-${name}`;
...
export function StudentEditFields({ student, photoUrl, submittedValues, fieldErrors }: { student: Student; photoUrl?: string | null; submittedValues?: StudentFormStateValues; fieldErrors?: StudentFieldErrors }) {
  ...
  return <>
    ...
    <section className="student-form-section"><header><p className="academic-kicker">Admission information</p><h2>Admission</h2></header><div className="student-form-grid"><PersonalInput student={s} name="admission_date" label="Admission date" type="date" required submittedValues={submittedValues} fieldErrors={fieldErrors}/><label>Student status<input value={s.status} readOnly/></label><PersonalInput student={s} name="previous_school_name" label="Previous school name" submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="previous_class" label="Previous class" submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="previous_school_tc_number" label="Previous school TC number" submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="previous_school_location" label="Previous school location" submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="admission_remarks" label="Remarks" submittedValues={submittedValues} fieldErrors={fieldErrors}/></div></section>
    ...
    <section className="student-form-section"><header><p className="academic-kicker">Additional information</p><h2>School details</h2></header><div className="student-form-grid"><PersonalInput student={s} name="identification_mark_1" label="Identification mark 1" submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="identification_mark_2" label="Identification mark 2" submittedValues={submittedValues} fieldErrors={fieldErrors}/><TransportRequired student={s} submittedValues={submittedValues} fieldErrors={fieldErrors}/><PersonalInput student={s} name="general_notes" label="General notes" submittedValues={submittedValues} fieldErrors={fieldErrors}/></div></section>
  </>;
}
